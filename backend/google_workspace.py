...
class EphemeralWorkspaceState:
    def __init__(self):
        self.creds_json = None      # Volatile service account JSON string
        self.admin_email = None      # Volatile admin email string
        self.use_synthetic = True    # Start disconnected by default

WORKSPACE_STATE = EphemeralWorkspaceState()


def _scan_workspace_api() -> list:
    """FR-1.1–1.2: Real Google Workspace Admin SDK enumeration."""
    admin_email = WORKSPACE_STATE.admin_email
    creds_json = WORKSPACE_STATE.creds_json

    if not admin_email or not creds_json:
        raise ValueError("Google Workspace is not connected in this session. Please upload credentials.")

    try:
        info = json.loads(creds_json)
        creds = service_account.Credentials.from_service_account_info(info, scopes=SCOPES)
    except Exception as e:
        logger.error("Failed to parse Google credentials from memory: %s", e)
        raise e
