...
import { Crosshair, Play, ShieldAlert, CheckCircle2, ChevronRight, ChevronDown, Wifi, WifiOff, Send, AlertTriangle, Shield, Zap, Eye, Lock } from 'lucide-react'
...
export default function RedTeamSimulator({ wsConnected }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'tester' && (
        !wsConnected ? (
          <div className="bg-zinc-900/40 border border-zinc-800 border-dashed rounded-2xl p-12 text-center w-full">
            <Lock className="w-12 h-12 text-zinc-600 mx-auto mb-3" />
            <p className="text-zinc-400 font-medium">Workspace Not Connected</p>
            <p className="text-sm text-zinc-500 mt-1">Please connect your Google Workspace in the top-right menu to enable the Live Prompt Injection Tester.</p>
          </div>
        ) : (
      ...
      {activeTab === 'simulation' && (
        !wsConnected ? (
          <div className="bg-zinc-900/40 border border-zinc-800 border-dashed rounded-2xl p-12 text-center w-full">
            <Lock className="w-12 h-12 text-zinc-600 mx-auto mb-3" />
            <p className="text-zinc-400 font-medium">Workspace Not Connected</p>
            <p className="text-sm text-zinc-500 mt-1">Please connect your Google Workspace in the top-right menu to run AI supply-chain attack simulations.</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {runs.length === 0 ? (
              ...
              <div className="space-y-4">
                {runs.map((run, i) => {
                  const report = typeof run.report === 'string' ? (() => { try { return JSON.parse(run.report) } catch { return {} } })() : (run.report || {})
